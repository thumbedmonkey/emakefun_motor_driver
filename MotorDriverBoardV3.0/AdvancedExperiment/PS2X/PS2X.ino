...
Emakefun_Sensor *ps2x = mMotorDriver.getSensor(E_PS2X);
...
void loop()
{
  ...
  if (ps2x->mPs2x->ButtonDataByte()) {
    ...
    if (ps2x->mPs2x->Button(PSB_L1) || ps2x->mPs2x->Button(PSB_R1)) {
      Serial.print("Stick Values:");
      Serial.print(ps2x->mPs2x->Analog(PSS_LY), DEC); //Left stick, Y axis. Other options: LX, RY, RX  
      Serial.print(",");
      Serial.print(ps2x->mPs2x->Analog(PSS_LX), DEC); 
      Serial.print(",");
      Serial.print(ps2x->mPs2x->Analog(PSS_RY), DEC); 
      Serial.print(",");
      Serial.println(ps2x->mPs2x->Analog(PSS_RX), DEC); 
    }
  } 
  delay(50);
}
